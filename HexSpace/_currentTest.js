...
{
    console.log("UPDATE")
    //if selected thing exists, follow ing smoothly
    if(exists(globals.world.selectedEntity));
    {
        ...
        globals.camera.offset.x += (globals.camera.targetOffset.x-globals.camera.offset.x)/16;
        globals.camera.offset.y += (globals.camera.targetOffset.y-globals.camera.offset.y)/16;
        console.log( globals.camera.offset,globals.camera.targetOffset)
...
//TODO Aim and shoot and detroy targets 
//ship.addOrder(new Order(attack,{target:entity},"Attack Asteroid","", "Player right clicked asteroid"));
function attack(ship,data)
{
    //if the path is done repath to target
    ...
    {
        ship.pathPoints.push(data.target.pos.clone());
    }
    data.callCount++;
    return (ship.pathPoints.length==1 && ship.pos.distance(data.pos)<= Math.max(ship.pathPointDetectRange*1.5,ship.radius)); //return if done or not
}
...
class Ship extends Entity 
{
    ...
    //this adds orders in the ship order list
    addOrder(order)
    {
    ...
    onDrawFluff(e) 
    {
        ...
        if(options.DEBUG.DRAWAIDETAILS.checked)
        {
            ...
            if(this.thrustToIntercept.mag_squared()>0)
            {
                ...
                drawArrow(e.ctx, this.pos.x, this.pos.y, vec2.x, vec2.y, 7/(e.camera.zoomFactor/e.camera.zoomFactor), "red", 2/e.camera.zoomFactor);
            }
            ...
            if(this.thrustCombined.mag_squared()>0)
            {
                ...
                drawArrow(e.ctx, this.pos.x, this.pos.y, vec2.x, vec2.y, 6/(e.camera.zoomFactor/e.camera.zoomFactor), "blue", 1.5/e.camera.zoomFactor);
            }
            ...
            if(this.thrustToAvoid.mag_squared()>0)
            {
                ...
                drawArrow(e.ctx, this.pos.x, this.pos.y, vec2.x, vec2.y, 5/(e.camera.zoomFactor/e.camera.zoomFactor), "yellow", 1/e.camera.zoomFactor);
            }
        }
...
class Bullet extends Entity 
{
  constructor() {
    ...
    this.setShape(scaleShape(5,[[0,0],[1,0],[0,0]]));
...
function myGameControlCode(e)
{
    ...
    if(!exists(globals.world.leftClickedEntities))
    {
        //add our globals to the global object and sub objects
        globals.world.leftClickedEntities = []; //the things the user clicked
        globals.world.rightClickedEntities = []; //the things the user clicked
    }
    ...
    if (exists(inputs.MOUSE.event.clientX))
    {
        ...
        {
            globals.world.leftClickedEntities = globals.world.entitiesAtPos(mousePos);
            //the list of entities the mouse clicked has something
            if (globals.world.leftClickedEntities.length)
            {
                //we clicked on something(s)!!!
                const entity = globals.world.leftClickedEntities[0];
                
                //if the entity is a ship, to switch scripts for human control
                ...
                if(globals.world.selectedEntity.id !== entity.id)
                {
                    ...
                    globals.camera.targetOffset.setXY(0,0);
                }
            }
        }
        //
        if (inputs.MOUSE.RIGHT.pressed())
        {
            //get the mouse pos
            const mousePos = e.camera.portToWorldPos(vec2.setXY(inputs.MOUSE.event.clientX,inputs.MOUSE.event.clientY));
            globals.world.rightClickedEntities = globals.world.entitiesAtPos(mousePos);
            if (globals.world.rightClickedEntities.length)
            {
                if(exists(globals.world.selectedEntity) && entity.id != globals.world.selectedEntity.id && globals.world.selectedEntity.entityType =="SHIP")
                {
                    const entity = globals.world.rightClickedEntities[0];
                    //clicked asteroid?
                
                    if(entity.entityType == "ASTEROID")
                    {
                        //get the mouse pos
                        globals.world.selectedEntity.addOrder(new Order(attack,{target:entity},"ATTACK","Mouse Attack Order", "Player right clicked asteroid"));
                    }
                    if(entity.entityType == "SHIP")
                    {
                        //get the mouse pos
            ...
            else //clicked background
            {
                if(exists(globals.world.selectedEntity) && globals.world.selectedEntity.entityType == "SHIP")
                {
                    //get the mouse pos
                    const mousePos = e.camera.portToWorldPos(vec2.setXY(inputs.MOUSE.event.clientX,inputs.MOUSE.event.clientY));
                    globals.world.selectedEntity.addOrder(new Order(goTo,{pos:mousePos},"WAYPOINT","Mouse Goto", "Player right clicked there"));
                }
            }
        }
    }
    //update ui properties window with the data of the first entity in the leftClickedEntities array
    if(globals.world.leftClickedEntities.length)
    {
        const entity = globals.world.leftClickedEntities[0];
        if(exists(entity))
