//07-ship-control-ai.js
// 
//  This file shows how to change ship control from one ship to another by clicking it, using script switching.
//      Clicking a ship will allow you to control it while the other ship goes to random spot in the world.
//
// CHANGE 1 (ctnd from last)
//      Improved the waypoint path resolving math and made the smooth fade after a key point to 
//      lower the amout of smooting down the path. the AI does a smooth every time it remove a path point
//      and, like the previous file, every nth step, if there was a point that shifted position.

//      Also used a faster fade of .6 in in the smooth point feature
//
//      Changed all mentions of "waypoint" to "pathPoint" instead. 
//          as Waypoints will be at a higher level of control.
//
//CHANGE 2
//      Added align to grid for way points to limit the shaking of the points around things
//      did not work to stabilize the waypoints tightning around the asteroid, turned it off 
//
//CHANGE 3
//      Added a second ship and turned the controlling into a script
//      The ships are controlled by changing scripts and so I remove the
//      mouse movement system and made a script for it
//      Made a random movement script for ai
//
//CHANGE 4 
//      Cleaned out excess comments in the fixWaypoints 
//
//CHANGE 5
//  Script Switching:
//      Added the ability to drive the AI ship by selecting it with the mouse
//
    ...
    if (exists(inputs.MOUSE.event.clientX))
    {
        ...
        if (inputs.MOUSE.LEFT.pressed() || inputs.MOUSE.LEFT.downTime() > 500)
        {
            ...
            //if (htmlelement.id === "canvas")
            {
                ...
                {
                    ship.clickedThings = foundThings;
                    
                }

                if (foundThings.length)
                {
                    console.log("Select",foundThings[0])
                    //CHANGE 5
...
class Ship extends Entity 
{
    constructor() 
    {
        ...
        //CHANGE 5. added entityType to entity class, and here it's set to SHIP
        //Identifiers
    ...
    onDrawFluff(e) 
    {
        ...
        if(this.clickedThings.length)
        {
            ...
            if(exists(e))
            {
                ...
                options.SELECTEDENTITY.TYPE.text = "" + e.entityType;
