//06-improved-path-smoothing.js
// 
// CHANGE 1 Improved the waypoint path resolving math and made the smooth fade after a key point to 
...
class Ship extends Entity 
{
    constructor() 
    {
        ...
        this.waypointDetectRangeMax  = this.radius*4
        this.waypointDetectRangeMin = this.radius*2
    ...
    fixWaypoints()
    {
        ...
        { //it actually only does one point, more stable results, see the return at the end of this
            ...
            if(atPos.length)
            {
                // the function calculated the distance point
    ...
    onThinkSelf() 
    {
        if(this.fixWaypointsCt<=0)
        {
            ...
            const waypointFixedAt = this.fixWaypoints()
            if(waypointFixedAt)
            {
                this.waypoints = smoothPointsVect2sKeepEdgesFractionFadeAfter(this.waypoints, 2,.1,waypointFixedAt);
            }
        ...
        if (this.waypoints.length > 0) 
        {
            ...
            {
                this.waypoints.shift();
                this.waypoints = smoothPointsVect2sKeepEdgesFractionFadeAfter(this.waypoints, 2,.1,3);
            }
