...
class Hex {
  ...
  class FractionalHex {
    ...
    static hexLinedraw(a, b) {
        const N = a.distance(b);
        const aNudge = new FractionalHex(a.q + 1e-06, a.r + 1e-06, a.s - 2e-06);
        const bNudge = new FractionalHex(b.q + 1e-06, b.r + 1e-06, b.s - 2e-06);
        const results = [];

        const step = 1.0 / Math.max(N, 1);

        for (let i = 0; i <= N; i++) {
            results.push(aNudge.hexLerp(bNudge, step * i).hexRound());
        }
...
class OffsetCoord {
	...
	static qoffsetFromCube(offset, h) {
	  const col = h.q;
	  const row =
		h.r + Math.floor((h.q + offset * (h.q & 1)) / 2);
	...
	static qoffsetToCube(offset, h) {
	  const q = h.col;
	  const r =
		h.row - Math.floor((h.col + offset * (h.col & 1)) / 2);
	  const s = -q - r;
	  if (offset !== OffsetCoord.EVEN && offset !== OffsetCoord.ODD) {
		throw new Error("offset must be EVEN (+1) or ODD (-1)");
	  }
	...
	static roffsetFromCube(offset, h) {
	  const col =
		h.q + Math.floor((h.r + offset * (h.r & 1)) / 2);
	  const row = h.r;
	  if (offset !== OffsetCoord.EVEN && offset !== OffsetCoord.ODD) {
		throw new Error("offset must be EVEN (+1) or ODD (-1)");
	  }
	...
	static roffsetToCube(offset, h) {
	  const q =
		h.col - Math.floor((h.row + offset * (h.row & 1)) / 2);
	  const r = h.row;
	  const s = -q - r;
	  if (offset !== OffsetCoord.EVEN && offset !== OffsetCoord.ODD) {
		throw new Error("offset must be EVEN (+1) or ODD (-1)");
  ...
  class DoubledCoord {
	...
	qdoubledToCube() {
	  const q = this.col;
	  const r = Math.floor((this.row - this.col) / 2);
	  const s = -q - r;
	  return new Hex(q, r, s);
	}
	...
	rdoubledToCube() {
	  const q = Math.floor((this.col - this.row) / 2);
	  const r = this.row;
	  const s = -q - r;
	  return new Hex(q, r, s);
	}
...
class Tests {
	...
	static testHexRound() {
		let a = new FractionalHex(0.0, 0.0, 0.0);
		let b = new FractionalHex(1.0, -1.0, 0.0);
		let c = new FractionalHex(0.0, -1.0, 1.0);
		Tests.equalHex(
		  "hex_round 1",
		  new Hex(5, -10, 5),
	  ...
	  static testLayout() {
		let h = new Hex(3, 4, -7);
		let flat = new Layout(Layout.flat, new Point(10.0, 15.0), new Point(35.0, 71.0));
		Tests.equalHex(
		  "layout",
		  h,
		  flat.pixelToHex(flat.hexToPixel(h)).hexRound()
		);
		let pointy = new Layout(Layout.pointy, new Point(10.0, 15.0), new Point(35.0, 71.0));
		Tests.equalHex(
		  "layout",
		  h,
		  pointy.pixelToHex(pointy.hexToPixel(h)).hexRound()
		);
	  }
